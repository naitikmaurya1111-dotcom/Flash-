...
// Resolve Firebase configuration with environment variables support for maximum production flexibility
...
const resolvedFirebaseConfig = {
  ...
  firestoreDatabaseId: metaEnv.VITE_FIREBASE_FIRESTORE_DATABASE_ID || firebaseConfig.firestoreDatabaseId || undefined,
};
...
// Interface for detailed Firestore diagnostics
...
// Global Standardized Firebase Firestore Error Handler
export function handleFirestoreError(error: unknown, operationType: OperationType, path: string | null) {
  ...
  console.error("Firestore Critical Error wrapped diagnostics: ", JSON.stringify(errInfo));
  throw new Error(JSON.stringify(errInfo));
}

// Validate Firebase Connection using getDocFromServer
export async function validateFirestoreConnection(): Promise<boolean> {
  try {
    ...
    if (error instanceof Error && error.message.includes("client is offline")) {
      console.warn("Firestore validation failed: Client is offline or permission disallowed.");
    }
    return false;
  }
}
